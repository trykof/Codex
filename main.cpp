...
template<class T>
        ...
        enum TOKENTYPE {
                        ...
                        PARSE_EXPRESSION, GLOBAL, LOCAL, TYPE, SIZE_KEYWORD
                    };
...
const std::unordered_map<std::string, TOKENTYPE> keywords = {
        ...
        {"local", LOCAL}, {"root", ROOT}, {"type", TYPE}, {"size", SIZE_KEYWORD}
};
...
void abs_m(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    stack.clear();
    if(arg.type() == R::NUMBER)
    {
        if(arg.number() < 0)
            stack.push_back(-arg.number());
        else stack.push_back(arg.number());
    }
    else return;
}

void pow_m(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    auto arg1 = stack[2];
    stack.clear();
    if(arg.type() == R::NUMBER && arg1.type() == R::NUMBER)
        stack.push_back(pow(arg.number(), arg.number()));
    else return;
}

void sqr(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    stack.clear();
    if(arg.type() == R::NUMBER)
        stack.push_back(arg.number()*arg.number());
    else return;
}

void exit_p(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
...
void random(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg1 = stack[1];
    auto arg2 = stack[2];
    stack.clear();
    if(arg1.type() == R::NUMBER && arg2.type() == R::NUMBER)
        stack.push_back((int)arg1.number()+rand()%(int)arg2.number());
    else return;
}

void gettime(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    ...
    {
        if(arg.string() == "time")
            stack.push_back(std::to_string(now->tm_hour) + ":" + std::to_string(now->tm_min) + ":" + std::to_string(now->tm_sec));
        else if(arg.string() == "date")
            stack.push_back(std::to_string(now->tm_year+1900) + "." + std::to_string(now->tm_mon+1) + "." + std::to_string(now->tm_mday));
    }
    else return;
}
...
void f_read(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    auto arg1 = stack[2];
    stack.clear();
    if(arg.type() == R::STRING && arg1.type() == R::NUMBER)
    {
        std::ifstream in = std::ifstream(arg.string());
        if(!in.is_open()) return;
        std::string line;
        std::vector<std::string> st;
        while(std::getline(in, line)) st.push_back(line);
        in.close();
        stack.push_back(st[arg1.number()]);
    }
    else return;

}

void try_open(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    ...
    if(arg.type() == R::STRING)
    {
        std::ifstream stream = std::ifstream(arg.string());
        if(stream.is_open()) stack.push_back(1);
...
void fsize(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    ...
    if(arg.type() == R::STRING)
    {
        ...
        while(std::getline(in, line)) st.push_back(line);
        in.close();
        stack.push_back(st.size());
    }
    else return;
}

void f_write(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    auto arg1 = stack[2];
    stack.clear();
    if(arg.type() == R::STRING && arg1.type() == R::STRING)
    {
        std::ofstream out = std::ofstream(arg.string());
...
void setcolor(std::vector<Interpreter::R>& stack)
{
    typedef Interpreter::R R;
    if (stack.empty()) return;
    auto arg = stack[1];
    ...
    if (arg.type() == R::NUMBER) {
        ...
        SetConsoleTextAttribute(hConsole, arg.number());
    }
    else return;
}

void str_at(std::vector<Interpreter::R> &stack)
{
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
...
void delay_m(std::vector<Interpreter::R>& stack) {
    typedef Interpreter::R R;
    if (stack.empty()) return;
    auto arg = stack[1]; 
    stack.clear();
...
void fcreate(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    stack.clear();
    if(arg.type() == R::STRING) {
        std::ofstream out(arg.string().c_str());
    }
    else return;
}
...
std::chrono::time_point<std::chrono::system_clock> stpoint;

void tmstart(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    stack.clear();
    stpoint = std::chrono::system_clock::now();
    return;
}

void tmstop(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    stack.clear();
    auto end = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsed_seconds = end-stpoint;
    std::time_t end_time = std::chrono::system_clock::to_time_t(end);
    stack.push_back(elapsed_seconds.count());
    return;
}

void sha256_m(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    stack.clear();
    if(arg.type() == R::STRING) {
        stack.push_back(sha256(arg.string()));
    }
    else return;
}

void fappend(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    auto arg1 = stack[2];
    ...
    if(arg.type() == R::STRING && arg1.type() == R::STRING) {
        std::ofstream stream;
        stream.open(arg.string(), std::ios_base::app);
        stream << arg1.string();
        return;
    }
    else return;
...
void settext(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    auto arg1 = stack[2];
    auto arg2 = stack[3];
    stack.clear();
    if(arg.type() == R::NUMBER && arg1.type() == R::NUMBER && arg2.type() == R::STRING) {
        COORD pos = {(short)arg.number(), (short)arg1.number()};
        HANDLE hConsole_c = CreateConsoleScreenBuffer( GENERIC_READ | GENERIC_WRITE, 0, NULL, CONSOLE_TEXTMODE_BUFFER, NULL);
        SetConsoleActiveScreenBuffer(hConsole_c);
        const char *str = std::string(arg2.string() + "\r\n").c_str();
...
void start(std::vector<Interpreter::R> &stack) {
    typedef Interpreter::R R;
    if(stack.empty()) return;
    auto arg = stack[1];
    stack.clear();
    if(arg.type() == R::STRING) {
        system(std::string("start " + arg.string()).c_str());
        return;
    }
    else return;
}
...
std::string containsIncludes(std::string text)
{
    auto spl = split(text);
    for (int i = 0; i < spl.size(); ++i) {
        std::string line = unescape(spl[i]);
        if(line == "#use")
        {
            if(i != spl.size())
            {
                std::ifstream ex(unescape(spl[i+1]));
                if(ex.good()) return &"." [ i];
                else return unescape(spl[i+1]);
            }
            else return "re";
        }
    }
...
void parseIncludes(std::string &form)
{
    std::string unescaped = form;
    unescaped = unescape(unescaped);
    std::string ci = containsIncludes(form);
    if(ci[0] != '.' && ci != "re") std::exception(std::string("Using " + ci + " not found.").c_str());
    else if (ci == "re") std::exception(std::string("Regex error while importing using.").c_str());
    else
    {
        auto spl = split(unescaped);
...
int main(int argc, char *argv[]) {
    try {
        ...
        interpreter.addFunction("string_cast", tostring);
        interpreter.addFunction("num_cast", tonumber);
        interpreter.addFunction("rand", random);
        interpreter.addFunction("exit", exit_p);
        interpreter.addFunction("sqr", sqr);
        interpreter.addFunction("abs", abs_m);
        interpreter.addFunction("pow", pow_m);
        interpreter.addFunction("fread", f_read);
        interpreter.addFunction("fwrite", f_write);
        interpreter.addFunction("ftry", try_open);
        interpreter.addFunction("fsize", fsize);
        interpreter.addFunction("strat", str_at);
        interpreter.addFunction("gettime", gettime);
        interpreter.addFunction("delay", delay_m);
        interpreter.addFunction("setcolor", setcolor);
        interpreter.addFunction("sha256", sha256_m);
        interpreter.addFunction("tmstart", tmstart);
        interpreter.addFunction("tmstop", tmstop);
        interpreter.addFunction("fcreate",fcreate);
        interpreter.addFunction("fappend", fappend);
        interpreter.addFunction("settext", settext);
        interpreter.addFunction("start", start);
        if(argc < 2) {
